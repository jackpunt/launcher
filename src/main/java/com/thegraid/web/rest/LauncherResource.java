...
public class LauncherResource {
    ...
    @Value("${thegraid.wssPath}")
    static String wssPath;

    @Value("${thegraid.lobbyUrl}")
    String lobbyUrl;

    private static String makeWssUrl(String hostUrl) {
        if (hostUrl != null) try {
            URL hostURL = new URL(hostUrl);
            URL wssURL = new URL("wss", hostURL.getHost(), hostURL.getPort(), wssPath);
            return wssURL.toString();
        } catch (MalformedURLException e) {
            // assert: we don't believe this can happen as we create hostUrl to be parseable.
            log.warn("could not parse hostUrl", e);
        }
        return null;
    }
    ...
    private LaunchResults launch(IGameInstDTO gameInst, String resultTicket) {
        ...
        log.info("Try launch(giid={1})", giid);
        log.info("Launch(giid={}): gameInst={}, propertyMap={}", giid, gameInst, gameInst.getPropertyMap());
        ...
        if (started != null) {
            ...
            results.setWssURL(makeWssUrl(hostUrl));
            ...
            String gameCtlUrl = lobbyUrl + "results/" + giid, wssUrl;
            results.setHostURL(gameCtlUrl);
            results.setWssURL(wssUrl = makeWssUrl(gameCtlUrl));
            started = game.start().toInstant(); // TODO: fix when game.start() is Instant
            results.setStarted(game.start().toInstant());
            log.warn("New launch: game started: {} @ {} on {} wss:{}", game, started, gameCtlUrl, wssUrl);
            if (started == null) {
